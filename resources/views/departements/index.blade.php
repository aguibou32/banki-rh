                                ...
                                <tbody>
                                    @foreach ($départements as $département)
                                        <tr>
                                            ...
                                            <td>{{ $département->description }}</td>
                                            <td>{{ $département->division->name }}</td>
                                            <td>{{ $département->services->count() }}</td>
                                            <td>{{ $département->created_at->formatLocalized('%d %B %Y') }}</td>

                                            @can('éditer départements')
                                                <td>
                                                    <a href="{{ route('departements.edit', $département->id) }}"
                                                        class="btn btn-transparent"><i class="bi bi-pencil-square"></i></a>
                                                    <form method="POST"
                                                        action="{{ route('departements.destroy', $département->id) }}">
                                                        @csrf
                                                        @method('DELETE')
